profil.controller('Membre', ['$scope', '$http', function($scope, $http){
  ...
  // requete HTTP en GET (via l'url)
    ...
    $http.get('https://jsonblob.com/api/57ac76f7e4b0dc55a4ec2ce5').success(function(response) { //reponse: contenu récupéré depuis l'url
         $scope.listeMembre = listeMembre = response; 
...
if (SessionJsonRecup("profilCheck")) {
  $scope.infoProfilActuel = SessionJsonRecup("profilCheck");
}

$scope.afficheProfil = function(member){
  $scope.infoProfilActuel = member;
  SessionJsonSet("profilCheck",$scope.infoProfilActuel)
}
